...
Board.prototype.detect = function (callback) {
  this.log('win-duino'.grey, 'attempting to find Arduino board on windows');
  var self = this;
  var found = false,
      err = null,
      temp;
  ...
  if (!err) {
	found = temp;
	self.log('win-duino'.grey,'found board at ' + this.device+ " with "+this.baudrate);	
  } else {
 ...
 */
 Board.prototype.open = function(){	
	this.log('win-duino'.yellow,'opening port...');
	if(this.isOpen){ this.log('win-duino'.yellow,'port already Openned'); return this.isOpen;}
	var board = this;
	this.serial.open(function (error) {
		if ( error ) {
			board.log('win-duino'.red,'error to open');
		} else {
			board.isOpen = true;
			board.log('win-duino'.green,'port openned');
		}
	});
...
Board.prototype.write = function (m) {	
  if (this.serial) {	
	...
	try{		
		...
		this.log('error'.red, 'Error Writing, buffering message: ' + m.red);
